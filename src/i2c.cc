...
int waitForBytes(int byteCount)
{
	int timeout = 1000 * byteCount;
...
Handle<Value> ReadByte(const Arguments& args) {
  ...
  if(waitForBytes(1) == -1)
  {
  	err = Exception::Error(v8::String::New("Cannot read device"));
  }
  else
  {
  	int res = wire->read();
...
Handle<Value> ReadBlock(const Arguments& args) {
  ...
  uint8_t cmd = args[0]->Int32Value();
  uint32_t len = args[1]->Int32Value();
  uint8_t data[len];
  Local<Value> err = Local<Value>::New(Null());
...
Handle<Value> WriteByte(const Arguments& args) {
  ...
  uint8_t byte = args[0]->Int32Value();
  Local<Value> err = Local<Value>::New(Null());
